...
- показывает только чаты, инициированные за последний час;
- если нужно посмотреть старые retained-чаты, передайте `chats: [...]`.

Пример запроса для конкретных чатов:
...
- считается по всем retained completed сообщениям всех пользователей сервера.
